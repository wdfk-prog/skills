---
name: 04-process-code-documentation
description: Use when adding or updating code documentation, especially when new C/C++ code needs Doxygen-format English comments for functions, structs, enums, file headers, or critical code blocks, or when authoring README/ADR/API docs; keywords: doxygen, comment, file header, struct, enum, function
---
...
Use Doxygen comments in English for:
- New functions, structs, and enums
- Important code blocks that need rationale, constraints, or algorithm notes
 ...
 * 2026-01-26 1.0     wdfk-prog   first version
 */
```
...
```c
/**
 * @brief Summarize what the function does in one sentence.
 * @param input Brief description of the parameter.
 * @param[out] output What the function writes.
 * @return 0 on success; non-zero error code on failure.
 * @note Constraints, side effects, or timing assumptions.
 */
int do_work(const Input *input, Output *output);
```

#### Structs and Enums
```c
/**
 * @brief Configuration for the driver.
 */
typedef struct DriverConfig {
  int baud_rate; /**< UART baud rate in bps. */
...
typedef enum DriverState {
  DRIVER_STATE_INIT = 0, /**< Initial state. */
  DRIVER_STATE_READY,    /**< Ready for I/O. */
  DRIVER_STATE_ERROR     /**< Error latched. */
} DriverState;
```
...
```c
/**
 * @brief Rationale for this block (why it exists).
 * @note Assumptions, constraints, or references.
 */
...
6. **Use Doxygen for C/C++** - Add English comments for new functions/structs/enums, file headers, and critical blocks
