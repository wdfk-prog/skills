---
name: 02-domain-arm-cortex-expert
description: >
  用于 ARM Cortex-M 平台的嵌入式固件/驱动开发与疑难排查：中断驱动 I/O、DMA/Cache 一致性、内存屏障、
  外设驱动与可靠性/性能优化；当做 MCU bring-up、外设驱动、HardFault/时序问题排查时使用。
metadata:
  model: inherit
---
...
- If detailed examples are required, ask for target MCU/RTOS/toolchain constraints and provide an end-to-end example inline.
