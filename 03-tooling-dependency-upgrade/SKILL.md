---
name: 03-tooling-dependency-upgrade
description: 用于规划与实施重大依赖升级（兼容性分析、分阶段升级、回退策略与测试矩阵）；当升级框架/工具链/库并处理 breaking change 时使用。
---
...
- SemVer/兼容性：先确认 breaking changes/peer deps/运行时要求
- 分阶段升级：一次只动一类依赖（框架/工具链/库）并可回退
- 测试策略：单测/集成/回归 + 关键链路冒烟 + 监控告警验证
