---
name: 03-tooling-git-advanced-workflows
description: 用于高级 Git 工作流（rebase/cherry-pick/bisect/worktree/reflog）以保持干净历史并从误操作中恢复；当处理复杂分支、回滚/找回提交或排查引入 bug 的提交时使用。
---
...
- Rebase/历史改写：只在私有分支做；共享分支优先 merge/revert
- 冲突处理：先理解差异→再按语义合并→最后跑测试验证
- 清理习惯：分支命名、定期清理已合并分支、保持 PR 小而清晰
